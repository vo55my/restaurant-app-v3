...
const createRestoItemTemplate = (restaurants) => `
  ...
  <img data-src="${CONFIG.BASE_IMAGE_URL + restaurants.pictureId}" alt="${restaurants.name}" class="card-img lazyload">
  <h3 class="card-title">${restaurants.name}</h3>
  <div class="info">
    <p class="rating"><img data-src="/icons/star.png" alt="location" class="star lazyload"/>${restaurants.rating}</p>
    <p class="lokasi"><img data-src="/icons/pin.png" alt="location" class="star lazyload"/>${restaurants.city}</p>
  </div>
